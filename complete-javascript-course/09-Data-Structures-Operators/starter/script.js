...
for(const message of flightList){
    const [status,from,to,time] = message.split(';');
    console.log(`${status.startsWith('_Delayed') ? '🚨': ''} ${status.replaceAll('_',' ').trim()} from ${from.slice(0,3).toUpperCase()} to ${to.slice(0,3).toUpperCase()} (${time.replace(':', 'h')})`);
}
// const flightList = flights.split('+');
